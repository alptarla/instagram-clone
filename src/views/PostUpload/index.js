...
const PostUpload = () => {
  ...
  return (
    <div className={classes.upload}>
      ...
      <div {...getRootProps()} className={classes.uploadContent}>
        <input {...getInputProps()} />
        <InboxOutlined />
        <p>Drag 'n' drop some files here, or click to select files</p>
        {preview ? <img width={200} src={preview} alt={description} /> : null}
      </div>
      <Input.TextArea
      ...
      />
      <Button type="primary" onClick={uploadPost} disabled={!file} loading={loading}>
        Upload Post
      </Button>
    </div>
  )
